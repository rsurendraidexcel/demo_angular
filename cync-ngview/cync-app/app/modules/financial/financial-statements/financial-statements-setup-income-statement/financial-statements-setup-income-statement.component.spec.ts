...
import { FinancialStatementsSetupIncomeStatementComponent } from './financial-statements-setup-income-statement.component';
...
describe('FinancialStatementsSetupIncomeStatementComponent', () => {
	let component: FinancialStatementsSetupIncomeStatementComponent;
	let fixture: ComponentFixture<FinancialStatementsSetupIncomeStatementComponent>;
	...
	const nullValueControl = new FormControl(null);
	...
	class MockRouter {
		...
		public re = new RoutesRecognized(2, '/financial/financial-statement/balancesheet/123',
			'/financial/financial-statement/balancesheet/123', null);
		...
		'incomeStatementData': {
			...
			'opProfitParams': [
				...
				},
				{
					...
					'id': '0c547860-abac-11e8-b6f2-0e141bdeec71',
					'uniqueId': 'OPERATING_COSTS_AND_EXPENSES1',
					'incomeParamType': 'EXPENSE',
					'customParameter': false
				},
				{
					...
					'id': '0c547860-abac-11e8-b6f2-0e141bdeec72',
					'uniqueId': 'OPERATING_COSTS_AND_EXPENSES2',
					'incomeParamType': 'EXPENSE',
					'customParameter': false
				},
				{
	...
	const deletedExpenseDataObject = {
		'parameter': 'Test',
		'incomeParamType': 'EXPENSE',
		'customParameter': true,
		'id': 101,
	...
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			...
			declarations: [FinancialStatementsSetupIncomeStatementComponent],
			providers: [
				APIMapper,
				Helper,
				MessageServices,
	...
	beforeEach(() => {
		fixture = TestBed.createComponent(FinancialStatementsSetupIncomeStatementComponent);
	...
	it('Should execute finance income statement', () => {
		expect(component).toBeTruthy();
	});

	it('Should execute financial income statement component', inject([Router], (router: Router) => {
		...
		ServiceSpy.spyCompServiceAndReturnDataForMock(getBalanceSheetDataSpy, financialStatementData);
		...
		component.draftFinanceStatement();
		expect(component.draftFinanceStatement).toHaveBeenCalled();

		// Should call update Income Statement Total method
		spyOn(component, 'updateIncomeStatementTotal').and.callThrough();
		component.updateIncomeStatementTotal('2018', control);
		expect(component.updateIncomeStatementTotal).toHaveBeenCalled();

		// Cancel Button Click method redirect to financial statement balance sheet page
		...
		// Cancel Button Click method redirect to financial statement summary page
		component.previousURL = 'https://devrorapi.cyncsoftware.com/financial/financial-statement/summary/123';
		component.cancelButtonClick();
		expect(component.cancelButtonClick).toHaveBeenCalled();

		// Redirect method call
		spyOn(component, 'redirectMethod').and.callThrough();
		component.redirectMethod();
		expect(component.redirectMethod).toHaveBeenCalled();

		// Should call Add new Custom Field method
		spyOn(component, 'addCustomField').and.callThrough();
		component.addCustomField('opProfitParams', 'Income');
		expect(component.addCustomField).toHaveBeenCalled();

		component.addCustomField('opProfitParams', 'EXPENSE');
		expect(component.addCustomField).toHaveBeenCalled();

		component.addCustomField('profitBeforeTaxParams', 'Income');
		expect(component.addCustomField).toHaveBeenCalled();

		component.addCustomField('compIncomeParams', 'Income');
		expect(component.addCustomField).toHaveBeenCalled();

		// Should call update deleted custom field total value method
		spyOn(component, 'updateDeletedCustomFieldTotalValue').and.callThrough();
		// Income scenario
		component.updateDeletedCustomFieldTotalValue(deletedIncomeDataObject, 'opProfitParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		component.updateDeletedCustomFieldTotalValue(deletedIncomeDataObject, 'profitBeforeTaxParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		component.updateDeletedCustomFieldTotalValue(deletedIncomeDataObject, 'compIncomeParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		// Expense scenario
		component.updateDeletedCustomFieldTotalValue(deletedExpenseDataObject, 'opProfitParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		component.updateDeletedCustomFieldTotalValue(deletedExpenseDataObject, 'profitBeforeTaxParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		component.updateDeletedCustomFieldTotalValue(deletedExpenseDataObject, 'compIncomeParams');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();
		...
		component.updateCustomParamValue('0', 'testfield', dummyFormGroup);
		expect(component.updateCustomParamValue).toHaveBeenCalled();
		...
		// Should call delete custom param value method
		spyOn(component, 'deleteCustomField').and.callThrough();
		component.deleteCustomField(0, 'opProfitParams', dummyFormGroup);
	...
	it('Should execute financial income statement component with null condition', inject([Router], (router: Router) => {
		const getPeojectStatusSpy = spyOn(commonService, 'getServiceProjectStatus');
		ServiceSpy.spyCompServiceAndReturnDataForMock(getPeojectStatusSpy, financeProjectStatus);
		...
		// Should call update Income Statement Total method null value scenario
		spyOn(component, 'updateIncomeStatementTotal').and.callThrough();
		component.updateIncomeStatementTotal(null, nullValueControl);
		expect(component.updateIncomeStatementTotal).toHaveBeenCalled();
	}));

	it('Should call save income statement data and save button click', () => {
		component.financialStatementAPIResponse = financialStatementData;
		component.financialStatementAPIResponse.id = null;
		...
		spyOn(component, 'saveIncomeStatement').and.callThrough();
		component.saveIncomeStatement(financialStatementData);
		expect(component.saveIncomeStatement).toHaveBeenCalled();
		expect(saveFinancialStatementsSpy).toHaveBeenCalled();
	});

	it('Should call update income statement data and save button click', () => {
		component.financialStatementAPIResponse = financialStatementData;
		component.financialStatementAPIResponse.id = '123';
	...
	it('Should call register Reload Grid On Client Selection false condition',
	...
	it('Should call add new custom field click method', () => {
		spyOn(component, 'addNewCustomFieldClick').and.callThrough();
		component.addNewCustomFieldClick('opProfitParams');
		expect(component.addNewCustomFieldClick).toHaveBeenCalled();

		component.addNewCustomFieldClick('profitBeforeTaxParams');
		expect(component.addNewCustomFieldClick).toHaveBeenCalled();

		component.addNewCustomFieldClick('compIncomeParams');
		expect(component.addNewCustomFieldClick).toHaveBeenCalled();
	});
});
