...
import { FinancialStatementsSetupBalanceSheetComponent } from './financial-statements-setup-balance-sheet.component';
...
describe('FinancialStatementsSetupBalanceSheetComponent', () => {
	let component: FinancialStatementsSetupBalanceSheetComponent;
	let fixture: ComponentFixture<FinancialStatementsSetupBalanceSheetComponent>;
	...
	const projectDetails: any = {
		'id': 'd353cdec-84fd-475a-86ce-343da0ed4510',
			...
			'assets': {
				'nonCurrentData': [
					...
					},
					{
						...
						'id': '90eaa0e4-c778-4d4f-9bfa-29335dc451a2',
						'uniqueId': 'TESTTT_peCoknYQshuDHGnnMsGiiPBQaIcnH2',
						'customParameter': true
					},
					{
						'parameter': 'testtt',
				...
				'liabilities': {
					'nonCurrentData': [
						...
						},
						{
							...
							'id': '90eaa0e4-c778-4d4f-9bfa-29335dc451a3',
							'uniqueId': 'TESTTT_peCoknYQshuDHGnnMsGiiPBQaIcnH3',
							'customParameter': true
						},
						{
							'parameter': 'libtest',
							...
							'id': '90eaa0e4-c778-4d4f-9bfa-29335dc451a4',
							'uniqueId': 'TESTTT_peCoknYQshuDHGnnMsGiiPBQaIcnH4',
							'customParameter': true
						},
						{
				...
				'equity': {
					'equityData': [
						...
						},
						{
							...
							'id': '0c4e6aa6-abac-11e8-b6f2-0e141bdeec73',
							'uniqueId': 'PREFERRED_COMMON_STOCK_ISSUED_MINUS_UNISSUE3',
							'customParameter': false
						},
						{
							'parameter': 'equityTestField',
							...
							'id': '0c4e6aa6-abac-11e8-b6f2-0e141bdeec74',
							'uniqueId': 'PREFERRED_COMMON_STOCK_ISSUED_MINUS_UNISSUED4',
							'customParameter': false
						},
						{
	...
	beforeEach(async(() => {
		TestBed.configureTestingModule({
			...
			declarations: [FinancialStatementsSetupBalanceSheetComponent],
			providers: [
				APIMapper,
				Helper,
				MessageServices,
	...
	beforeEach(() => {
		fixture = TestBed.createComponent(FinancialStatementsSetupBalanceSheetComponent);
	...
	it('should create financial Balance sheet statement component', inject([Router], (router: Router) => {
		...
		const getBalanceSheetDataSpy = spyOn(financialStatementsService, 'getFinancialStatements');
		ServiceSpy.spyCompServiceAndReturnDataForMock(getBalanceSheetDataSpy, balanceSheetData);
		...
		expect(getBalanceSheetDataSpy).toHaveBeenCalled();
		...
		// Redirect method code coverage with url string and action type save
		component.previousURL = 'https://devrorapi.cyncsoftware.com/financial/financial-statement/123';
		spyOn(component, 'redirectPageMethod').and.callThrough();
		component.redirectPageMethod('save');
		expect(component.redirectPageMethod).toHaveBeenCalled();

		// Redirect method code coverage with url string empty and action type update
		component.previousURL = '';
		component.redirectPageMethod('update');
		expect(component.redirectPageMethod).toHaveBeenCalled();

		// Should call updateAssetsSum method
		spyOn(component, 'updateAssetsSum').and.callThrough();
		component.updateAssetsSum('2018', control);
		expect(component.updateAssetsSum).toHaveBeenCalled();

		// Should call updateLiabilitiesSum method
		spyOn(component, 'updateLiabilitiesSum').and.callThrough();
		component.updateLiabilitiesSum('2018', control);
		expect(component.updateLiabilitiesSum).toHaveBeenCalled();

		// Should call updateEquitySum method
		spyOn(component, 'updateEquitySum').and.callThrough();
		component.updateEquitySum('2018', control);
		expect(component.updateEquitySum).toHaveBeenCalled();

		// Should call assets add new custom field condition
		spyOn(component, 'addCustomField').and.callThrough();
		component.addCustomField('assets', 'currentData', '');
		expect(component.addCustomField).toHaveBeenCalled();

		// Should call equityAndLiabilities add new custom field condition
		component.addCustomField('equityAndLiabilities', 'liabilities', 'currentData');
		expect(component.addCustomField).toHaveBeenCalled();

		// Should call assets current update deleted custom field total value condition
		spyOn(component, 'updateDeletedCustomFieldTotalValue').and.callThrough();
		component.updateDeletedCustomFieldTotalValue(deletedDataObject, 'assets', 'currentData', '');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		// Should call assets non current update deleted custom field total value condition
		component.updateDeletedCustomFieldTotalValue(deletedDataObject, 'assets', 'nonCurrentData', '');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		// Should call equityAndLiabilities current update deleted custom field total value condition
		component.updateDeletedCustomFieldTotalValue(deletedDataObject, 'equityAndLiabilities', 'liabilities', 'currentData');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		// Should call equityAndLiabilities non current update deleted custom field total value condition
		component.updateDeletedCustomFieldTotalValue(deletedDataObject, 'equityAndLiabilities', 'liabilities', 'nonCurrentData');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();

		// Should call equityAndLiabilities equity data update deleted custom field total value condition
		component.updateDeletedCustomFieldTotalValue(deletedDataObject, 'equityAndLiabilities', 'equity', 'equityData');
		expect(component.updateDeletedCustomFieldTotalValue).toHaveBeenCalled();
		...
		component.updateCustomParamValue('0', 'testtt', dummyFormGroup);
		expect(component.updateCustomParamValue).toHaveBeenCalled();

		// Duplicate cusotm parameter validation for libality
		component.updateCustomParamValue('0', 'libtest', dummyFormGroup);
		expect(component.updateCustomParamValue).toHaveBeenCalled();

		// Duplicate cusotm parameter validation for equiry
		component.updateCustomParamValue('0', 'equityTestField', dummyFormGroup);
		expect(component.updateCustomParamValue).toHaveBeenCalled();
		...
		component.deleteCustomField(0, 'assets', 'currentData', '', dummyFormGroup);
		expect(component.deleteCustomField).toHaveBeenCalled();

		// Should call delete custom param value method for equity and liabilities
		component.deleteCustomField(0, 'equityAndLiabilities', 'liabilities', 'currentData', dummyFormGroup);
		expect(component.deleteCustomField).toHaveBeenCalled();
	...
	it('Should click on cancel button and redirect back: url string is undefined', () => {
		spyOn(component, 'cancelButtonClick').and.callThrough();
		component.cancelButtonClick();
		expect(component.cancelButtonClick).toHaveBeenCalled();
	});
	...
	it('should save new Balance sheet data and \'Save\' button is clicked', () => {
		...
		spyOn(component, 'saveBalanceSheet').and.callThrough();
		component.saveBalanceSheet(balanceSheetData);
		expect(component.saveBalanceSheet).toHaveBeenCalled();
		expect(saveFinancialStatementsSpy).toHaveBeenCalled();

	});

	it('should update the Balance sheet data and \'Save\' button is clicked', () => {
		...
		component.saveBalanceSheet(balanceSheetData);
		expect(component.saveBalanceSheet).toHaveBeenCalled();
		expect(updateFinancialStatementsSpy).toHaveBeenCalled();
	});
