...
import { Country, State, NaicsCodeCombined, NaicsCode, CorporateType, Guarantors, UpdateRequestBody } from '../model/guarantors.model.component'
...
*/
@Component({
  selector: 'app-guarantors-add',
  templateUrl: './add.guarantors.component.html'
})
export class GuarantorsAddComponent {
  addEditGuarantors: FormGroup;
  lenderId: string;
  ...
  */
  initializeForm() {
    this.addEditGuarantors = this.fb.group({
  ...
  _renderForm() {
    this.route.params.subscribe(params => {
      ...
      if (this.RecordId !== undefined && this.RecordId !== 'add') {
        ...
        this.addEditGuarantors.controls['country_id'].setValue(null);
        // this.addEditGuarantors.controls['naics_code_id'].setValue(null);
        this.addEditGuarantors.controls['state_province_id'].setValue(null);
        this.addEditGuarantors.controls['corporate_type'].setValue(null);
      }
    });
  ...
  */
  showDetailsInForm() {
    this._guarantorsService.getDetails(this._apiMapper.endpoints[CyncConstants.UPDATE_CLIENT_LOAN_GUARANTORS].replace("{clientId}", this.borrowerId) + "/" + this.RecordId).subscribe(user => {
      this.addEditGuarantors.patchValue(user, { onlySelf: true });
  ...
  */
  getAllCountries() {
    this._guarantorsService.setCountriesListApiEndpoint();
    this._guarantorsService.getCountriesList().subscribe(countriesResp => {
  ...
  */
  onChangeOfCountry() {
    this._guarantorsService.setStatesListApiEndpoint(this.addEditGuarantors.controls['country_id'].value);
  ...
  */
  getAllStatesByCountryId() {
    this._guarantorsService.getStatesListBasedOnSelectedCountry().subscribe(statesResp => {
  ...
  */
  enableOrDisableStateDropDown() {
    if (this.statesList != undefined && this.statesList.length > 0) {
      this.addEditGuarantors.controls['state_province_id'].enable();
    } else {
      this.addEditGuarantors.controls['state_province_id'].disable();
    }
  }
  ...
  */
  getAllNaicsCode() {
    this._guarantorsService.setNaicsCodeListApiEndpoint();
    this._guarantorsService.getNaicsCodeList().subscribe(naicsResp => {
  ...
  */
  getCorporateType() {
    this._guarantorsService.setCotporateListApiEndpoint();
    this._guarantorsService.getCorporateList().subscribe(Resp => {
  ...
  */
  saveData(model: Guarantors, isValid: boolean, isSavenCont: boolean) {
  ...
  * @param - model, isVslid, isSavenCont
  */
  saveDetails(model: Guarantors, isValid: boolean, isSavenCont: boolean) {
    if (isValid) {
      this._message.showLoader(true);
      let requestBody = new UpdateRequestBody();
      requestBody.guarantor = model;
      this._guarantorsService.saveNew(this._apiMapper.endpoints[CyncConstants.UPDATE_CLIENT_LOAN_GUARANTORS].replace("{clientId}", this.borrowerId), requestBody).subscribe(res => {
        this._message.addSingle(CyncConstants.SAVE_MESSAGE, "success");
        if (!isSavenCont) {
          this.navigateToHome();
        } else {
          this.addEditGuarantors.reset();
          this.addEditGuarantors.controls['state_province_id'].disable();
          this.addEditGuarantors.controls['naics_code_id'].setValue("");
          this._message.showLoader(false);
        }
      })
    }
  ...
  * @param - model
  */
  updateRecordDetails(model: Guarantors) {
    this._message.showLoader(true);
    let requestBody = new UpdateRequestBody();
    requestBody.guarantor = model;
    this._guarantorsService.updateDetails(this._apiMapper.endpoints[CyncConstants.UPDATE_CLIENT_LOAN_GUARANTORS].replace("{clientId}", this.borrowerId) + "/" + this.RecordId, requestBody).subscribe(res => {
  ...
  */
  navigateToHome() {
    this._router.navigateByUrl("/client-maintenance/client-loan-terms/guarantors");
  }
  ...
  */
  displayFieldCss(field: string) {
    return this._helper.getFieldCss(field, this.addEditGuarantors)
  }
  ...
  */
  getFormControl(field: string) {
    return this.addEditGuarantors.get(field);
  }
