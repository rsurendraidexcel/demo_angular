...
@Component({
	selector: 'app-bucket-aging',
	templateUrl: './bucket-aging.component.html',
	styleUrls: ['./bucket-aging.component.scss']
})
export class BucketAgingComponent implements OnInit {
	...
	constructor(
        ...
        */
		...
		this.clientTemplatesService.getTemplateViewType().subscribe(
			data => {
				this.templateViewtype = data;
			}
		);

		this.clientTemplatesService.getTemplateIdForPost().subscribe(
			data => {
				this.id = data;
			}
		);
		this.getBuckingAgingDetails();
	}

	ngOnInit() {
		this.disablewhenView();
	}
 ...
 */
	...
	/**
    * Initialize Bucket Aging List thru service
    */
	getBuckingAgingDetails() {
		this.clientTemplatesService.sendAllTemplateData().subscribe(response => {
			this.arBucketAgingRowData = response.bucket_ageings.data;
			this.apBucketAgingRowData = response.payable_bucket_ageings.data;
			this.bucketName = response.retention_label;
    ...
    */
	...
	/**
	 * This method reset whole page of bucket aging
	 */
	cancelBucketAgingData() {
		const popupParams = { 'title': 'confirmation', message: 'Please save changes, or do you want to reset it ?' }
		this.helper.openConfirmPopup(popupParams).subscribe(result => {
			if (result === true) {
				const url = this.apiMapper.endpoints[CyncConstants.BUCKET_AGING_LIST].replace('{id}', this.id);
				this.clientTemplatesService.getBucketAgingService(url).subscribe(response => {
	...
	saveBucketAgingData() {
		...
		const url = this.apiMapper.endpoints[CyncConstants.SAVE_BUCKET_AGING_DATA].replace('{id}', this.id);
		this.clientTemplatesService.saveBucketAgingService(url, requestBody).subscribe(response => {
			this.isDataLoaded = true;
			this.message.showLoader(false);
			const message = 'Bucket Aging has been successfully updated';
			this.helper.showApiMessages(message, 'success');
			this.getBuckingAgingData();
