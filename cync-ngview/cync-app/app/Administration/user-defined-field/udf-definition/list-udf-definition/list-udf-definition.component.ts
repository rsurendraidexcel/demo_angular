...
import { UDFDefinition } from '@app/Administration/user-defined-field/udf-definition/models/udf-definition.model';
...
@Component({
	selector: 'app-list-udf-definition',
	templateUrl: './list-udf-definition.component.html',
	styleUrls: ['./list-udf-definition.component.scss']
})
export class ListUdfDefinitionComponent implements OnInit {
	...
	showSpinner: boolean;
	dataTableElementId = CyncConstants.P_DATA_TABLE_UDF_DEFINITION_ID;
	...
	getDataTableElementId() {
		return CyncConstants.P_DATA_TABLE_UDF_DEFINITION_ID;
	}
	...
	*/
	initializeGrid() {
		...
		this.gridModel = {
			...
			apiDef: {
				getApi: this._apiMapper.endpoints[CyncConstants.GET_UDF_DEFINITION_LIST],
				deleteApi: this._apiMapper.endpoints[CyncConstants.DELETE_UDF_DEFINITION],
				updateApi: '',
				exportApi: this._apiMapper.endpoints[CyncConstants.EXPORT_UDF_DEFINITION]
			},
			type: CyncConstants.UDF_DEFINITION_SUMMARY,
			columnDef: [
				{
					field: 'name', header: CyncConstants.UDF_NAME,
					sortable: true, isTemplate: false, templateHtml: '', hidden: false, filter: true
				},
				{
					field: 'description', header: CyncConstants.UDF_DESCRIPTION,
					sortable: true, isTemplate: false, templateHtml: '', hidden: false, filter: true
				},
				{
					field: 'field_type', header: CyncConstants.UDF_FIELD_TYPE,
					sortable: true, isTemplate: false, templateHtml: '', hidden: false, filter: true
				},
				{
					field: 'validation_type', header: CyncConstants.UDF_VALIDATION_TYPE,
					sortable: true, isTemplate: false, templateHtml: '', hidden: false, filter: true
				},
				{
					field: 'status', header: CyncConstants.UDF_STATUS,
					sortable: true, isTemplate: false, templateHtml: '', hidden: false, filter: true
				}
			],
			responseKey: CyncConstants.UDF_DEFINITION_LIST_RESPONSE_KEY,
			...
			searchViaAPI: true,
			menuName: CyncConstants.UDF_DEFINITION_MENU_NAME,
	...
	getGridData(url: string) {
		this._udfDefinitionService.getUDFGridData(url).subscribe(response => {
	...
	*/
	getPermissions() {
		...
		if (!this._helper.compareIgnoreCase(roleType, CyncConstants.ADMIN_ROLE_TYPE)) {
			this._udfDefinitionService.getMenuPermission(this.gridModel.menuName).subscribe(permissionsResponse => {
	...
	*/
	...
	showConfirmationPopup() {
		...
		this._helper.openConfirmPopup({
			...
			'message': this._udfDefinitionService.getConfirmationPopupMessage(this.selectedRows, 'name'),
			'msgType': 'warning'
		}).subscribe(result => {
			if (result) {
	...
	deleteSelectedRecords() {
		...
		this._udfDefinitionService.deleteUDF(apiEndpoint).subscribe(deleteResponse => {
	...
	exportSelectedColumns(selectedColumns: ExportColumn[]) {
		...
		if (queryParam !== undefined && queryParam.length > 0) {
			...
			this.toogleExportConfirmPoupupFlag();
			this._message.showLoader(true);
			this._udfDefinitionService.exportData(queryParam, apiEndpoint).subscribe(blob => {
	...
	*/
	onScroll(event) {
		...
		if (this.load_more && this.gridData.length < this.recordTotal) {
			...
			this._udfDefinitionService.getUDFGridData(endpoint).subscribe(apiResponse => {
